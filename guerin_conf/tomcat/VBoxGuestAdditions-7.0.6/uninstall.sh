...
# Read routines.sh
if ! test -r "/opt/VBoxGuestAdditions-7.0.6/routines.sh"; then
    echo 1>&2 "Required file routines.sh not found.  Aborting..."
    return 1
fi
. "/opt/VBoxGuestAdditions-7.0.6/routines.sh"
...
echo 1>&2 "Removing installed version 7.0.6 of VirtualBox Guest Additions..."
...
if test -r "/opt/VBoxGuestAdditions-7.0.6"/init/vboxadd-service; then
...
if test -r "/opt/VBoxGuestAdditions-7.0.6"/init/vboxadd; then
    stop_init_script vboxadd 2>> "$LOGFILE"
    test -n "$NO_CLEANUP" || "/opt/VBoxGuestAdditions-7.0.6"/init/vboxadd cleanup 2>> "$LOGFILE"
